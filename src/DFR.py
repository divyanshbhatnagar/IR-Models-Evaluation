...
with open('D:\\Project 3\\test_query.txt', encoding="utf-8") as f:
    ...
    for line in f:
        ...
        inurl = 'http://localhost:8983/solr/dfr/select?fl=id,score&indent=on&q=' + query + '&defType=dismax&pf=text_en^2%20+%20text_de^2%20+%20text_ru^2&qf=text_en^2%20+%20text_de^2%20+%20text_ru^2&rows=20&wt=json'
        "qid = count"
        IRModel = 'DFR'
        ...
        if count<10:
            ...
            qid='0' + str(count)
        for doc in docs:
            outf.write(str(qid) + ' ' + 'Q' + str(count) + ' ' + str(doc['id']) + ' ' + str(rank) + ' ' + str(doc['score']) + ' ' + 'DFR' + '\n')
            rank += 1
        outf.close()
        count += 1
